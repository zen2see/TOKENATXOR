...
contract EscrowFacet is Modifiers {
    ...
    /// @notice Allow the deposit of an ERC20 token to the escrow contract of a claimed tokenatxor
    /// @dev Will throw if token being deposited is same as collateral token for the tokenatxor
    ...
    /// @notice Allow the deposit of multiple ERC20 tokens to the escrow contract of a multiple claimed tokenatxors
    /// @dev Will throw if one of the tokens being deposited is same as collateral token for the corresponding tokenatxor
    ...
    /// @notice Allow the deposit of TKTR into the escrow of multiple tokenatxors
    /// @param _tokenIds An array containing the identifiers of the NFTs receiving TKTR
    /// @param _values An array containing the amounts of ERC20 tokens to deposit into each tokenatxor
    function batchDepositTKTR(uint256[] calldata _tokenIds, uint256[] calldata _values) external {
        require(_tokenIds.length == _values.length, "EscrowFacet: TokenIDs and Values length must match");

        address maticTKTR = 0x385Eeac5cB85A38A9a07A70c73e0a3271CfB54A7;
        for (uint256 index = 0; index < _tokenIds.length; index++) {
            uint256 tokenId = _tokenIds[index];
            uint256 value = _values[index];
            address escrow = s.tokenatxors[tokenId].escrow;
            require(escrow != address(0), "EscrowFacet: Does not have an escrow");
            /// emit Erc20Deposited(tokenId, maticGHST, value, LibMeta.msgSender(), escrow);
            LibERC20.transferFrom(maticTKTR, LibMeta.msgSender(), escrow, value);
        }
    }
    ...
    function transferEscrow(
    ...
    ) external onlyTokenatxorOwner(_tokenId) onlyUnlocked(_tokenId) {
        address escrow = s.tokenatxors[_tokenId].escrow;
        address collateralType = s.tokenatxors[_tokenId].collateralType;
        require(escrow != address(0), "EscrowFacet: Does not have an escrow");
        ...
        if (IERC20(_erc20Contract).allowance(escrow, address(this)) < _transferAmount) {
            CollateralEscrow(escrow).approveTokenatxorDiamond(_erc20Contract);
        }
