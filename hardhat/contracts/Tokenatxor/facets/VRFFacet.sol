...
contract VrfFacet is Modifiers {
    ...
    event OpenProductions(uint256[] _tokenIds);
    event ProductionOpened(uint256 indexed tokenId);
   ...
   |__________________________________*/
    ...
    function openProducions(uint256[] calldata _tokenIds) external {
        address owner = LibMeta.msgSender();
        for (uint256 i; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(s.tokenatxors[tokenId].status == LibTokenatxor.STATUS_CLOSED_PRODUCTION, "TokenatxorFacet: Production is not closed");
            require(owner == s.tokenatxors[tokenId].owner, "TokenatxorFacet: Only tokenatxor owner can open a portal");
            require(s.tokenatxors[tokenId].locked == false, "TokenatxorFacet: Can't open portal when it is locked");
            drawRandomNumber(tokenId);
            LibERC721Marketplace.cancelERC721Listing(address(this), tokenId, owner);
        }
        emit OpenProductions(_tokenIds);(_tokenIds);
    }

    function drawRandomNumber(uint256 _tokenId) internal {
        s.tokenatxors[_tokenId].status = LibTokenatxor.STATUS_VRF_PENDING;
        uint256 fee = s.fee;
    ...
    function tempFulfillRandomness(bytes32 _requestId, uint256 _randomNumber) internal {
        ...
        require(s.tokenatxors[tokenId].status == LibTokenatxor.STATUS_VRF_PENDING, "VrfFacet: VRF is not pending");
        s.tokenatxors[tokenId].status = LibTokenatxor.STATUS_OPEN_PRODUCTION;
        s.tokenIdToRandomNumber[tokenId] = _randomNumber;
        emit ProductionOpened(tokenId);
