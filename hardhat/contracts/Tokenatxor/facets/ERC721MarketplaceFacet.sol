...
import {LibTokenatxor, TokenatxorInfo} from "../libraries/LibTokenatxor.sol";
...
contract ERC721MarketplaceFacet is Modifiers {
    ...
    /// @notice Get an tokenatxor listing details through an identifier
    ...
    /// @return tokenatxorInfo_ A struct containing details about the tokenatxor
    function getTokenatxorListing(uint256 _listingId) external view returns (ERC721Listing memory listing_, TokenatxorInfo memory tokenatxorInfo_) {
        listing_ = s.erc721Listings[_listingId];
        require(listing_.timeCreated != 0, "ERC721Marketplace: ERC721 listing does not exist");
        tokenatxorInfo_ = LibTokenatxor.getTokenatxor(listing_.erc721TokenId);
    }
    ...
    /// @param _category Category of listings to query 0 == production, 1 == vrf pending, 1?2 == open production, 2?3 == Tokenatxor.
    ...
    struct TokenatxorListing {
        ERC721Listing listing_;
        TokenatxorInfo tokenatxorInfo_;
    }

    /// @notice Query a certain amount of tokenatxor listings created by an address based on their category and sortings
    /// @param _owner Creator of the listings to query
    /// @param _category Category of listings to query 0 == production, 1 == vrf pending, 1?2 == open production, 2?3 == Tokenatxor.
    /// @param _sort Sortings of listings to query "listed" or "purchased"
    /// @param _length How many aavegotchi listings to return
    /// @return listings_ An array of structs, each struct containing details about each listing being returned
    function getOwnerTokenatxorListings(
        address _owner,
        uint256 _category,
        string memory _sort,
        uint256 _length // how many items to get back or the rest available
    ) external view returns (TokenatxorListing[] memory listings_) {
        uint256 listingId = s.erc721OwnerListingHead[_owner][_category][_sort];
        listings_ = new TokenatxorListing[](_length);
        uint256 listIndex;
        for (; listingId != 0 && listIndex < _length; listIndex++) {
            ERC721Listing memory listing = s.erc721Listings[listingId];
            listings_[listIndex].listing_ = listing;
            listings_[listIndex].tokenatxorInfo_ = LibTokenatxor.getTokenatxor(listing.erc721TokenId);
            listingId = s.erc721OwnerListingListItem[listingId].childListingId;
        }
        assembly {
            mstore(listings_, listIndex)
    ...
    function getERC721Listings(
        uint256 _category, // 0 == produciton, 1 == vrf pending, 1 == open produciton, 2 == Tokenatxor.
    ...
    /// @notice Query a certain amount of tokenatxor listings
    /// @param _category Category of listings to query  0 == production, 1 == vrf pending, 1?2 == open production, 2?3 == Tokenatxor.
    /// @param _sort Sortings of listings to query
    /// @param _length How many listings to return
    /// @return listings_ An array of structs, each struct containing details about each listing being returned
    function getTokenatxorListings(
        uint256 _category, // 0 == production, 1 == vrf pending, 1 == open produciton, 2 == Tokenatxor
        string memory _sort, // "listed" or "purchased"
        uint256 _length // how many items to get back or the rest available
    ) external view returns (TokenatxorListing[] memory listings_) {
        uint256 listingId = s.erc721ListingHead[_category][_sort];
    ...
    /// @return category_ Category of the NFT  0 == production, 1 == vrf pending, 1?2 == open production, 2?3 == Tokenatxor.
    function getERC721Category(address _erc721TokenAddress, uint256 _erc721TokenId) public view returns (uint256 category_) {
        ...
        category_ = s.tokenatxors[_erc721TokenId].status; // 0 == portal, 1 == vrf pending, 2 == open portal, 3 == Aavegotchi
    }
    ...
    function addERC721Listing(
        ...
        require(_priceInWei >= 1e18, "ERC721Marketplace: price should be 1 TKTR or larger");
        ...
        require(category != LibTokenatxor.STATUS_VRF_PENDING, "ERC721Marketplace: Cannot list a production that is pending VRF");
        ...
        s.tokenatxors[_erc721TokenId].locked = true;
        ...
        if (s.listingFeeInWei > 0) {
            ...
            LibERC20.transferFrom(s.tktrContract, owner, address(0xFFfFfFffFFfffFFfFFfFFFFFffFFFffffFfFFFfF), s.listingFeeInWei);
        }
    }
    ...
    function executeERC721Listing(uint256 _listingId) external {
        ...
        require(IERC20(s.tktrContract).balanceOf(buyer) >= priceInWei, "ERC721Marketplace: not enough TKTR");
        ...
        uint256 artistsShare = (priceInWei * 2) / 100;
        // AGIP6 adds on 0.5%
        uint256 playerRewardsShare = priceInWei / 200;
        uint256 transferAmount = priceInWei - (daoShare + artistsShare + playerRewardsShare);
        LibERC20.transferFrom(s.tktrContract, buyer, s.artists, artistsShare);
        LibERC20.transferFrom(s.tktrContract, buyer, s.daoTreasury, daoShare);
        LibERC20.transferFrom(s.tktrContract, buyer, seller, transferAmount);
        /// AGIP6 adds on 0.5%
        LibERC20.transferFrom((s.tktrContract), buyer, s.rarityFarming, playerRewardsShare);

        s.tokenatxors[listing.erc721TokenId].locked = false;

        /// To do (Nick) -- Explain why this is necessary
        if (listing.erc721TokenAddress == address(this)) {
            LibTokenatxor.transfer(seller, buyer, listing.erc721TokenId);
        } else {
            /// TKTRStakingDiamond
