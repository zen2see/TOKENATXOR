...
contract ERC1155MarketplaceFacet is Modifiers {
    ...
    /// @return listing_ A struct containing certain details about the ERC1155 listing associated with an NFT
    /// of contract address `_erc721TokenAddress` and identifier `_erc721TokenId`
    ...
    /// @param _category Category of listings to query // 0 is equippable, 1 is badge, 2 is consumable, 3 is tickets
    ...
    function getERC1155Listings(
        uint256 _category, // 0 is equippable, 1 is badge, 2 is consumable, 3 is tickets
    ...
    /// @notice Allow the tokenatxor diamond owner or DAO to set the default listing fee
    ...
    /// @notice Allow the tokenatxor diamond owner or DAO to set the category details for multiple ERC1155 NFTs
    ...
    /// @return category_ Category of the NFT // 0 is equippable, 1 is badge, 2 is consumable, 3 is tickets
    ...
    function setERC1155Listing(
        ...
        require(cost >= 1e18, "ERC1155Marketplace: cost should be 1 TKTR or larger");
        ...
        if (s.listingFeeInWei > 0) {
            ...
            LibERC20.transferFrom(s.tktrContract, LibMeta.msgSender(), address(0xFFfFfFffFFfffFFfFFfFFFFFffFFFffffFfFFFfF), s.listingFeeInWei);
        }
    }
    ...
    function executeERC1155Listing(
        ...
        listing.quantity -= _quantity;
        uint256 cost = _quantity * _priceInWei;
        require(IERC20(s.tktrContract).balanceOf(buyer) >= cost, "ERC1155Marketplace: not enough TKTR");
        {
            // handles stack too deep error
            uint256 daoShare = cost / 100;
            uint256 artistsShare = (cost * 2) / 100;
            // AGIP6 adds on 0.5%
            uint256 playerRewardsShare = cost / 200;

            uint256 transferAmount = cost - (daoShare + artistsShare + playerRewardsShare);
