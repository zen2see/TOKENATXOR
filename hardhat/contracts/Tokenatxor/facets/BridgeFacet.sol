...
contract BridgeFacet is Modifiers {
    ...
    event AddedTokenatxorBatch(address indexed owner, uint256[] tokenIds);
    ...
    /// @notice Allow the Tokenatxor Diamond owner or Dao to change the childChain manager address
    ...
    function withdrawTokenatxorBatch(uint256[] calldata _tokenIds) external {
        address owner = LibMeta.msgSender();
        require(_tokenIds.length <= 20, "Bridge: exceeds withdraw limit for single transaction");
        for (uint256 i; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            require(owner == s.tokenatxors[tokenId].owner, "BridgeFacet: Not owner of token");
            require(s.tokenatxors[tokenId].locked == false, "BridgeFacet: Can't withdraw locked token");
            LibTokenatxor.transfer(owner, address(this), tokenId);
        }
    ...
    function deposit(address _user, bytes calldata _depositData) external {
        ...
        if (tokenType == ERC1155_TOKEN_TYPE) {
            ...
            for (uint256 i; i < tokenIds.length; i++) {
                ...
                require(address(this) == s.tokenatxors[tokenId].owner, "Bridge: Not owner of token");
                LibTokenatxor.transfer(address(this), _user, tokenId);
            }
            emit AddedTokenatxorBatch(_user, tokenIds);
        }
    }
}