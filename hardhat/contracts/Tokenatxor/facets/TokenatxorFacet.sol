...
contract TokenatxorFacet {
    ...
    function balanceOf(address _owner) external view returns (uint256 balance_) {
        require(_owner != address(0), "TokenatxorFacet: _owner can't be address(0");
    ...
    ///@return tokenatxorInfo_ a struct containing all details about
    function getTokenatxor(uint256 _tokenId) external view returns (TokenatxorInfo memory tokenatxorInfo_) {
        tokenatxorInfo_ = LibTokenatxor.getTokenatxor(_tokenId);
    }
    ...
    function tokenatxorClaimTime(uint256 _tokenId) external view returns (uint256 claimTime_) {
        claimTime_ = s.tokenatxors[_tokenId].claimTime;
    }
    ...
    function tokenByIndex(uint256 _index) external view returns (uint256 tokenId_) {
        require(_index < s.tokenIds.length, "TokentatxorFacet: index beyond supply");
    ...
    function tokenOfOwnerByIndex(address _owner, uint256 _index) external view returns (uint256 tokenId_) {
        require(_index < s.ownerTokenIds[_owner].length, "TokenatxorFacet: index beyond owner balance");
    ...
    /// @param _owner The address to check for the NFTs
    /// @return tokenatxorInfos_ an array of structs,where each struct contains all the details of each NFT
    function allTokenatxorsOfOwner(address _owner) external view returns (TokenatxorInfo[] memory tokenatxorInfos_) {
        uint256 length = s.ownerTokenIds[_owner].length;
        tokenatxorInfos_ = new TokenatxorInfo[](length);
        for (uint256 i; i < length; i++) {
            tokenatxorInfos_[i] = LibTokenatxor.getTokenatxor(s.ownerTokenIds[_owner][i]);
        }
    }
    ...
    function ownerOf(uint256 _tokenId) external view returns (address owner_) {
        owner_ = s.tokenatxors[_tokenId].owner;
        require(owner_ != address(0), "TokenatxorFacet: invalid _tokenId");
    }
    ...
    function internalTransferFrom(
        ...
        require(
            ...
            "TokenatxorFacet: Not owner or approved to transfer"
        );
        LibTokenatxor.transfer(_from, _to, _tokenId);
    ...
    function approve(address _approved, uint256 _tokenId) external {
        address owner = s.tokenatxors[_tokenId].owner;
    ...
    function tokenURI(uint256 _tokenId) external pure returns (string memory) {
        return LibStrings.strWithUint("ipfs://", _tokenId); //Here is your URL!
        /// "https://zen2see.com/tokenatxor/metadata/items/";
    }
}
