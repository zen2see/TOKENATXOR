...
import {LibAppStorage, AppStorage, ItemType, Tokenatxor, EQUIPPED_SLOTS} from "./LibAppStorage.sol";
...
library LibItems {
    //EQUIPPABLES
    uint8 internal constant EQUIPPABLE_SLOT_BODY = 0;
    uint8 internal constant EQUIPPABLE_SLOT_FACE = 1;
    uint8 internal constant EQUIPPABLE_SLOT_EYES = 2;
    uint8 internal constant EQUIPPABLE_SLOT_HEAD = 3;
    uint8 internal constant EQUIPPABLE_SLOT_HAND_LEFT = 4;
    uint8 internal constant EQUIPPABLE_SLOT_HAND_RIGHT = 5;
    uint8 internal constant EQUIPPABLE_SLOT_PET = 6;
    uint8 internal constant EQUIPPABLE_SLOT_BG = 7;
    uint256 internal constant ITEM_CATEGORY_EQUIPPABLE = 0;
    uint256 internal constant ITEM_CATEGORY_BADGE = 1;
    uint256 internal constant ITEM_CATEGORY_CONSUMABLE = 2;
    uint8 internal constant EQUIPPABLE_SLOTS_TOTAL = 11;
    ...
    function removeFromParent(
        ...
        if (bal == 0) {
            ...
            if (_fromContract == address(this)) {
                checkEquippableIsEquipped(_fromTokenId, _id);
            }
        }
        if (_fromContract == address(this) && bal == 1) {
            Tokenatxor storage tokenatxor = s.tokenatxors[_fromTokenId];
            if (
                tokenatxor.equippedEquippables[LibItems.EQUIPPABLE_SLOT_HAND_LEFT] == _id &&
                tokenatxor.equippedEquippables[LibItems.EQUIPPABLE_SLOT_HAND_RIGHT] == _id
            ) {
    ...
    function checkEquippableIsEquipped(uint256 _fromTokenId, uint256 _id) internal view {
        AppStorage storage s = LibAppStorage.diamondStorage();
        for (uint256 i; i < EQUIPPED_SLOTS; i++) {
            require(s.tokenatxors[_fromTokenId].equippedEquippables[i] != _id, "Items: Cannot transfer EQUIPPABLE that is equipped");
        }
    }
}