...
const { equippablesSvgs, sleevesSvgs } = require("../../svgs/h2equippables.js");
...
async function uploadSvgs(svgs, svgType, ...{ }) {
  ...
  while (true) {
    ...
    console.log(`Uploading ${svgItemsStart} to ${svgItemsEnd} equippable SVGs`);
...
async function main() {
  ...
  await uploadSvgs(equippablesSvgs, "equippables", {
    gasPrice: gasPrice,
  });
  ...
  console.log("Associating sleeves svgs with body equippable svgs.");
