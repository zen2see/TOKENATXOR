...
async function batchMintProductions() {
  ...
  if (testing) {
   ...
   const signer: any = await ethers.provider.getSigner(itemManager);
    nonceManagedSigner = new NonceManager(signer);
  } else if (hre.network.name === "matic") {
    const signer = accounts[0]; // new ethers.Wallet(process.env.ITEM_MANAGER);
  } else {
  ...
  console.log(`Batch minting Productions to Item Manager: ${itemManager}`);
  ...
  while (remaining > 0) {
    ...
    console.log(`Minting ${numberPerMint} Productions of ${maxNumber}!`);
    let r = await shopFacet.mintProductions(itemManager, numberPerMint, {
      gasPrice: gasPrice,
    });
